...
import com.bayue.ciic.server.entity.AddVideoResponseEntity;
import com.bayue.ciic.server.entity.TranscodeResponseEntity;
import com.bayue.ciic.upload.constant.TranscodeType;
import com.bayue.ciic.upload.constant.UploadType;
import com.bayue.ciic.upload.model.UploadDbHelper;
import com.bayue.ciic.upload.model.UploadTotalDataAccessor;
import com.bayue.ciic.upload.model.VideoItem;
...
//            showNotification("上传已完成", true);
    ...
    private boolean uploadOne(final VideoItem videoItem) {
        ...
        try {
            Log.d(TAG, "uploadOne()   " + videoItem.getId());
            Log.i(TAG, "upload type：" + videoItem.getType());
            ...
            public void run() {
                if (videoItem.getType() == UploadType.SHORT_VIDEO) {
                    ...
                    DemoServerHttpClient.getInstance().addVideo(videoItem.getVid(), videoItem.getDisplayName(), 0, new DemoServerHttpClient.DemoServerHttpCallback<AddVideoResponseEntity>() {
                        @Override
                        public void onSuccess(AddVideoResponseEntity addVideoResponseEntity) {
                            if (addVideoResponseEntity.getTransjobstatus() == 0) {
