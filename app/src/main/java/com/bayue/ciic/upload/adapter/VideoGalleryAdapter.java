 ...
 */
public class VideoGalleryAdapter extends VcloudRecyclerViewAdapter<VideoGalleryAdapter.LineContainer> {
    ...
    public void onBindViewHolder(final VcloudViewHolder holder, final int position) {
        ...
        if(holder instanceof VideoDateTipHolder){
            ...
            for (int i = 0; i < 3; i++) {
                ...
                //当此行数据小于3时,隐藏多余项
                if(i >= lineContainer.videoItems.size()){
                    subHolder.itemView.setVisibility(View.INVISIBLE);
                    continue;
                }
                VideoItem videoItem = lineContainer.videoItems.get(i);
